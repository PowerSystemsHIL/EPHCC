  ...
  nrhs, const mxArray * prhs[] )
{
  ...
  if (strcmp(machineName, "CHP_Test_Model") == 0) {
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"CHP_Test_Model")) {
    return(sf_CHP_Test_Model_method_dispatcher(simstructPtr,chartFileNumber,
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfGlobalMdlStartCallCounts == 1) {
    ...
    if (!simModeIsRTWGen) {
      CHP_Test_Model_debug_initialize(sfGlobalDebugInstanceStruct);
    }
